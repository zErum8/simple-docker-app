    ...
    stage('Docker') {
        ...
        withDockerNetwork{ n ->
           docker.image('arminc/clair-db:latest').withRun("--network ${n} --name db") { c->
              docker.image('arminc/clair-local-scan:v2.0.8_fe9b059d930314b54c78f75afe265955faf4fdc1').inside("--network ${n} -p 6060:6060 --name clair --restart on-failure -config=/config/config.yaml") {
                sh '''
